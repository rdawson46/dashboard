...
const emit = defineEmits(['close', 'edit-job']);
...
onMounted(() => {
    if (props.job) {
        ...
        jobType.value = props.job.task.task_type;
        freq.value = props.job.freq || '15mins';

        if (props.job.task.task_type === 'LLM') {
            query.value = props.job.task.query || '';
        } else if (props.job.task.task_type === 'Tool') {
            // TODO: 
        }
    }
});
                    ...
                    <div v-else-if="jobType === 'Tool'">
                        ...
                        <Transition name="fade" mode="out-in">
                            <div v-if="toolType === 'code'">
                                <div class="form-group">
                                    <label for="code-upload">Upload New Code (optional)</label>
                                    <input type="file" id="code-upload" @change="handleFileUpload" accept=".py">
                                </div>
                            </div>
